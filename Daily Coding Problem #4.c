...
int main(int argc, char *argv[]) {
    ...
    int arrayC[] = {5, 1, 10, 5, 1, 2};
    int sizeC    = 6;
    
    printf("Sum @ %d\n", max_sum(arrayC, sizeC));
}
...
int max_sum(int array[], int size) {
    ...
    // untile the last element of the same array. This is the
    // first loop.
    for(int index = 0; index < size; index++) {
        
        // The second loop starts from index+2 because
        // we don't want the adjacent elements of the
        // current element, until the last element of
        // the same array.
        for(int jndex = index+2; jndex < size; jndex++) {
